...
def get_post(id: int):
    ...
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                detail=f"post wiht id: {id} not found")
...
def delete_post(id: int):
    # deleting post logic
    # in the list, find the index of the corresponing post's ID
    # my_posts.pop(index)
...
def update_post(id: int, post: Post):
    ...
    return {"message": f"Post with id: {id} has been succesfully updated with {post_dict}"}
